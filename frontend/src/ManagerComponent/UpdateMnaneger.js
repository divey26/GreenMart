...
function ManagerUpdateItem() {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                const response = await fetch(`http://localhost:8020/manager_item_order/${id}`);
    ...
    const handleUpdate = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8020/manager_item_update`, {
                method: 'PUT',
                headers: {
